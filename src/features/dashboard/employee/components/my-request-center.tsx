...
export function MyRequestCenter({ requests: propRequests }: { requests?: BackendRequestItem[] }) {
  ...
  const items = propRequests
    ? propRequests.map(r => ({
        ...
        label: r.category,
        sublabel: r.detail,
        status: ((r.status === 'Pending' || r.status === 'Review' || r.status === 'Approved' || r.status === 'Done' || r.status === 'Rejected') ? r.status : 'Pending') as RequestItem['status']
      }))
    : defaultRequests
          ...
          >
            ...
            <span
              className={cn(
                ...
                statusStyle[req.status] || 'text-amber-700 bg-amber-100 dark:text-amber-300 dark:bg-amber-900/40'
              )}
            >
              {req.status}
            </span>

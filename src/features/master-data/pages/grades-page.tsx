...
import { GradeModal } from '../components/add-edit-modals'

export function GradesPage() {
  const { data: list, isPending, error } = useGrades()
  const { mutateAsync: createGrade, isPending: isCreating } = useCreateGrade()
  const { mutateAsync: updateGrade, isPending: isUpdating } = useUpdateGrade()
  const { mutateAsync: deleteGrade } = useDeleteGrade()

  const [searchQuery, setSearchQuery] = useState('')
  const [selectedItem, setSelectedItem] = useState<Grade | undefined>()
  const [showModal, setShowModal] = useState(false)

  const gradesList = list ?? []
  ...
  const handleSave = async (payload: Omit<Grade, 'id'>) => {
    try {
      if (selectedItem) {
        await updateGrade({ id: selectedItem.id, updates: payload })
        snackbar.success('Grade berhasil diperbarui!')
      } else {
        await createGrade(payload)
        snackbar.success('Grade baru berhasil ditambahkan!')
      }
      setShowModal(false)
    } catch (err) {
  ...
  const handleDelete = async (item: Grade) => {
    if (confirm(`Apakah Anda yakin ingin menghapus grade ${item.name}?`)) {
      try {
        await deleteGrade(item.id)
        snackbar.success('Grade berhasil dihapus.')
      } catch (err) {
        snackbar.exception(err)
      }
    }
  ...
  return (
    <AppMain
      ...
      title='Grades'
      subtitle='Kelola golongan jabatan dan batasan nominal penggajian karyawan'
      actions={
        <button
          type='button'
          onClick={() => {
      ...
      <div className='mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
        <div className='relative flex-1 max-w-md'>
          ...
          <input
            ...
            placeholder='Cari grade berdasarkan kode atau nama...'
          ...
          <table className='w-full border-collapse text-left'>
            <thead>
              <tr className='border-b border-border bg-muted/30'>
                ...
                <th className='px-3 py-3.5 text-xs font-bold text-muted-foreground uppercase'>Kode Grade</th>
                <th className='px-3 py-3.5 text-xs font-bold text-muted-foreground uppercase'>Nama Grade</th>
                <th className='px-3 py-3.5 text-center text-xs font-bold text-muted-foreground uppercase'>Level Angka</th>
                <th className='px-3 py-3.5 text-right text-xs font-bold text-muted-foreground uppercase'>Range Gaji (Min - Max)</th>
              ...
              {filteredList.length === 0 ? (
                <tr>
                  <td colSpan={7} className='py-16 text-center text-xs text-muted-foreground'>
                    Tidak ada data grade yang ditemukan.
                  </td>
                </tr>
              ) : (
                filteredList.map((item, idx) => (
                  <tr key={item.id} className='group hover:bg-muted/10 transition-colors'>
                    ...
                    <td className='px-3 py-4 text-center text-xs text-foreground font-bold'>{item.level}</td>
                    <td className='px-3 py-4 text-right text-xs text-foreground font-mono'>
                      {formatRupiah(item.minSalary)} - {formatRupiah(item.maxSalary)}
                    </td>

...
import { HolidayModal } from '../components/add-edit-modals'

export function HolidaysPage() {
  const { data: list, isPending, error } = useHolidays()
  ...
  const [selectedItem, setSelectedItem] = useState<Holiday | undefined>()
  const [showModal, setShowModal] = useState(false)

  const holidaysList = list ?? []
  ...
  const handleSave = async (payload: Omit<Holiday, 'id'>) => {
    try {
      if (selectedItem) {
        await updateHoliday({ id: selectedItem.id, updates: payload })
        snackbar.success('Hari libur berhasil diperbarui!')
      } else {
        await createHoliday(payload)
        snackbar.success('Hari libur baru berhasil ditambahkan!')
      }
      setShowModal(false)
    } catch (err) {
  ...
  const handleDelete = async (item: Holiday) => {
    if (confirm(`Apakah Anda yakin ingin menghapus hari libur ${item.name}?`)) {
      try {
        await deleteHoliday(item.id)
        snackbar.success('Hari libur berhasil dihapus.')
      } catch (err) {
        snackbar.exception(err)
      }
    }
  ...
  return (
    <AppMain
      ...
      title='Holidays'
      subtitle='Kelola kalender hari libur nasional dan kebijakan libur perusahaan'
      actions={
        <button
          type='button'
          onClick={() => {
          ...
          <table className='w-full border-collapse text-left'>
            <thead>
              <tr className='border-b border-border bg-muted/30'>
                ...
                <th className='px-3 py-3.5 text-xs font-bold text-muted-foreground uppercase'>Nama Hari Libur</th>
                <th className='px-3 py-3.5 text-center text-xs font-bold text-muted-foreground uppercase'>Tanggal</th>
                <th className='px-3 py-3.5 text-center text-xs font-bold text-muted-foreground uppercase'>Jenis Libur</th>
                <th className='px-3 py-3.5 text-xs font-bold text-muted-foreground uppercase'>Keterangan</th>
              ...
              {filteredList.length === 0 ? (
                ...
                filteredList.map((item, idx) => (
                  <tr key={item.id} className='group hover:bg-muted/10 transition-colors'>
                    ...
                    <td className='px-3 py-4 text-xs font-bold text-foreground'>{item.name}</td>
                    <td className='px-3 py-4 text-center text-xs text-foreground font-mono font-semibold'>{item.date}</td>

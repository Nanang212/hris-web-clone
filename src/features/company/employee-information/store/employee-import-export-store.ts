// src/features/company/employee-information/store/employee-import-export-store.ts
...
export interface ExportColumnDefinition {
  id: string
  label: string
  category: 'profile' | 'personal' | 'employment' | 'status' | 'bank' | 'npwp' | 'bpjs' | 'documents'
  checked: boolean
}
...
export const INITIAL_EXPORT_COLUMNS: ExportColumnDefinition[] = [
  ...
  { id: 'bpjsKetenagakerjaan', label: 'BPJS Ketenagakerjaan Number', category: 'bpjs', checked: true },
  // Documents
...
const VALID_DEPARTMENTS = ['Product Design', 'Engineering', 'Finance', 'HR', 'Marketing', 'Operations', 'Sales', 'Legal']
...
export async function parseAndValidateEmployeeFile(
  file: File,
  mode: 'create' | 'bulk_update',
  existingEmployees: typeof INITIAL_MOCK_EMPLOYEES = useEmployeeImportExportStore.getState().employees,
  ...
  rawRows.forEach((row, index) => {
    ...
    const employeeId = rawEmployeeId || `EMP-${dayjs().format('YYYY')}-${String(index + 100).padStart(5, '0')}`
      ...
      if (mode === 'create') {
        ...
        if (existingWithEmail) {
          errors.push({
            ...
            value: email,
            solution: 'Employee data already exists in database. Use a unique email or switch to Bulk Update mode.',
          })
      ...
      if (mode === 'create') {
        ...
        if (existingWithId) {
          errors.push({
            ...
            value: rawEmployeeId,
            solution: 'Employee ID is already taken. Leave blank to auto-generate or use a unique ID.',
          })
    ...
    // 6. Department check
    if (department && !VALID_DEPARTMENTS.some((d) => d.toLowerCase() === department.toLowerCase())) {
      errors.push({
        row: rowNum,
        column: 'Department',

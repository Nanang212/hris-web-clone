// src/features/company/employee-information/pages/import-employee-information-page.tsx
import {
  IconAlertCircle,
  IconAlertTriangle,
  IconCircleCheck,
...
export function ImportEmployeeInformationPage() {
  ...
  const handleDownloadTemplate = () => {
    if (mode === 'create') {
      ...
      snackbar.success('Template pre-filled with existing employee data downloaded! Edit desired fields and re-upload.')
    }
  }
  ...
  const handleProcessFile = async (uploadedFile: File) => {
    ...
    try {
      ...
      setPreviewRows(result.previewRows)
      setRawValidRows(result.rawValidRows)
      snackbar.info(`File loaded: ${uploadedFile.name} (${result.summary.totalRows} rows). Review preview below and click Import Data to execute.`)
    } catch {
      ...
      <div className='mx-auto flex max-w-6xl flex-col gap-6 pb-12'>
        ...
        <div className='flex flex-col gap-2 rounded-2xl border bg-card p-4 shadow-xs sm:flex-row sm:items-center sm:justify-between'>
          <div className='flex flex-col gap-0.5'>
            <h3 className='text-sm font-semibold text-foreground'>
              {mode === 'create' ? 'Bulk Insert (Create New Employees)' : 'Bulk Update (Update Existing Employees)'}
            </h3>
              ...
              <CardContent className='flex flex-col gap-3'>
                {!file ? (
                  <div className='flex flex-col items-center justify-center py-6 text-center text-muted-foreground'>
                    ...
                    <p className='text-[11px]'>Upload a spreadsheet above to view error breakdown and validation checks.</p>
                  </div>
                ) : errors.length === 0 ? (
                  <div className='flex flex-col items-center justify-center py-6 text-center text-emerald-600 dark:text-emerald-400'>
                    ...
                    <p className='text-[11px] text-muted-foreground'>
                      No formatting or data errors found. All {summary.validRows} rows are ready to be imported.
                    </p>
                  </div>
                ) : (
                  <>
                        ...
                        <TableBody>
                          {errors.slice(0, 5).map((err, idx) => (
                            <TableRow key={idx} className='text-xs'>
                              <TableCell className='font-medium text-foreground'>{err.row}</TableCell>
                              <TableCell className='font-semibold text-foreground'>{err.column}</TableCell>
                              <TableCell className='text-red-600 dark:text-red-400'>{err.errorMessage}</TableCell>
                              <TableCell className='font-mono text-[11px] text-muted-foreground'>{err.value}</TableCell>
              ...
              <CardContent className='flex flex-col gap-4'>
                <div className='grid grid-cols-2 gap-3'>
                  ...
                  <div className='flex items-center justify-between rounded-2xl border bg-emerald-50/40 p-3.5 shadow-xs dark:bg-emerald-950/20'>
                    <div>
                      <p className='text-[11px] text-emerald-700 dark:text-emerald-400'>Valid Rows</p>
          ...
          <CardContent>
            {!file || previewRows.length === 0 ? (
              <div className='flex flex-col items-center justify-center py-10 text-center text-muted-foreground'>
                ...
                <p className='text-xs'>Upload an Excel or CSV file to inspect data rows before importing.</p>
              </div>
            ) : (
              <div className='overflow-auto rounded-xl border'>
                <Table>
        ...
        <div className='flex items-center justify-end gap-3'>
          <Button
            type='button'
            variant='outline'
            onClick={() => void navigate({ to: '/company/employee-info' })}
          >
            Cancel
          </Button>
          <Button
            ...
            className='gap-2 bg-blue-600 px-6 font-semibold text-white shadow-sm hover:bg-blue-700 dark:bg-blue-600 disabled:opacity-50'
          >
            {importing ? (
      ...
      <Dialog open={allErrorsOpen} onOpenChange={setAllErrorsOpen}>
        <DialogContent className='max-h-[85vh] overflow-y-auto sm:max-w-3xl'>
          <DialogHeader>
            ...
            <DialogDescription>
              Resolve these errors in your spreadsheet and re-upload, or proceed to import valid rows only.
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className='max-h-[85vh] overflow-y-auto sm:max-w-xl'>
          ...
          <div className='flex flex-col gap-3 text-xs text-muted-foreground'>
            <div className='rounded-xl border p-3'>
              ...
              <p className='mt-1'>Full Name, Email, Phone Number, Department, Position, and Join Date cannot be empty.</p>
            </div>
            <div className='rounded-xl border p-3'>
              <p className='font-semibold text-foreground'>Email Verification</p>
              <p className='mt-1'>Must follow standard RFC email format (e.g. username@domain.com).</p>
            </div>
            <div className='rounded-xl border p-3'>
              <p className='font-semibold text-foreground'>Tax Identification (NPWP)</p>
              <p className='mt-1'>Must be 15 or 16 numeric digits (format XX.XXX.XXX.X-XXX.XXX is supported).</p>
            </div>
            <div className='rounded-xl border p-3'>
              <p className='font-semibold text-foreground'>Bulk Update Rule</p>
              <p className='mt-1'>In Bulk Update mode, Employee ID must match existing records in the database.</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className='flex max-h-[88vh] flex-col overflow-hidden sm:max-w-4xl'>
            ...
            <div className='flex items-center justify-between gap-4 pr-6'>
              ...
              <div className='flex items-center gap-2'>
                ...
                <Badge
                  ...
                  className='text-xs'
                >
                  {summary.errorRows === 0 ? 'Ready to Import' : `${summary.errorRows} Errors Found`}
                </Badge>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className='grid grid-cols-4 gap-2 border-y py-2.5 text-center text-xs'>
            ...
            <div>
              ...
              <p className='font-bold text-emerald-600 dark:text-emerald-400'>{summary.validRows} rows</p>
            </div>
            <div>
              <p className='text-[11px] text-amber-600 dark:text-amber-400'>Warnings</p>
              <p className='font-bold text-amber-600 dark:text-amber-400'>{summary.warningRows} rows</p>
            </div>
            <div>
              ...
              <TableBody>
                {previewRows.map((row) => (
                  ...
                  >
                    ...
                    <TableCell className='font-mono font-medium text-foreground'>{row.employeeId}</TableCell>
                    ...
                    <TableCell>
                      <Badge variant={row.status === 'Active' ? 'emerald' : 'secondary'} className='text-[10px]'>
                        {row.status}
                      </Badge>
                    </TableCell>
                    <TableCell className='text-center'>
                      {row.isValid ? (
                        <Badge variant='emerald' className='text-[10px]'>Valid</Badge>
                      ) : (
          ...
          <div className='flex items-center justify-between pt-2'>
            <p className='text-xs text-muted-foreground'>
              Showing document preview. Click &ldquo;Import Data&rdquo; at the bottom of the page to execute the import.
            </p>
            <Button
              type='button'
              variant='outline'

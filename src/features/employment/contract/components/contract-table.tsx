...
export function ContractTable({
  ...
  const paginatedContracts = contracts.slice((activePage - 1) * pageSize, activePage * pageSize)
      ...
      <Table>
        ...
        <TableBody>
          ...
          {paginatedContracts.map((ctr, index) => {
            const status = STATUS_CONFIG[ctr.status as ContractStatus] ?? {
              label: ctr.status,
              variant: 'gray' as const,
            }
            return (
              <TableRow key={ctr.id} className='transition-colors hover:bg-muted/30'>

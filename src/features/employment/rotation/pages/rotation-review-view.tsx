// rotation-review-view.tsx — Review & Confirm page before submitting rotation
import { IconLoader2 } from '@tabler/icons-react'

import { Button } from '@/shared/components/ui/button'
import type { Employee } from '@/features/employment/rotation/types'

import type { RotationFormData } from './rotation-create-view'

interface RotationReviewViewProps {
  formData: RotationFormData
  employees: Employee[]
  onBack: () => void
  onSubmit: () => void
  isPending?: boolean
...
export function RotationReviewView({
  formData,
  employees,
  onBack,
  onSubmit,
  isPending = false,
}: RotationReviewViewProps) {
  const employee = employees.find((e) => e.id === formData.employeeId)

  const currentOrg = [employee?.divisionName, employee?.departmentName, employee?.positionName]
    .filter(Boolean)
    .join(' / ')
      ...
      <div className='overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm'>
        <div className='border-b border-border/60 px-6 py-4'>
          ...
          <p className='mt-0.5 text-xs text-muted-foreground'>
            Verify all details before submitting the rotation request.
          </p>
        </div>
        ...
        <div className='mx-6 mt-2 mb-6 rounded-xl border border-amber-200 bg-amber-50/60 px-4 py-3 dark:border-amber-800/40 dark:bg-amber-950/20'>
          <p className='text-xs text-amber-800 dark:text-amber-300'>
            Submitting starts the configured approval route. Assignment updates on the effective
            date after approval.
          </p>
        </div>
      </div>

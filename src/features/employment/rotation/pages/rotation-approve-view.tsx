// rotation-approve-view.tsx — Approval view: Before & After + Approve/Reject actions
...
interface RotationApproveViewProps {
  ...
  return (
    <div
      className={`flex flex-col gap-3 rounded-xl border p-4 ${
        highlight ? 'border-primary/20 bg-primary/5' : 'border-border/60 bg-muted/20'
      }`}
    >
      <p
...
export function RotationApproveView({
  rotation,
  onBack,
  onApprove,
  onReject,
  isPending = false,
}: RotationApproveViewProps) {
  const status = STATUS_CONFIG[rotation.status]
  const canApprove = rotation.status === 'pending'
      ...
      <div className='flex items-center justify-between rounded-2xl border border-border/60 bg-card px-6 py-4 shadow-sm'>
        <div className='flex flex-col gap-1'>
          ...
          <p className='text-sm font-bold text-foreground'>
            {rotation.fullName} · {rotation.employeeCode}
          </p>
        </div>
      ...
      <div className='overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm'>
        ...
        <div className='grid grid-cols-1 gap-4 p-6 sm:grid-cols-2'>
          <AssignmentCard
            ...
            items={[
              { label: 'Division', value: rotation.currentDivision },
              { label: 'Department', value: rotation.currentDepartment },
              { label: 'Position', value: rotation.currentPosition },
              { label: 'Supervisor', value: rotation.currentSupervisor },
              { label: 'Location', value: rotation.currentLocation },
            ]}
          />
          <AssignmentCard
            ...
            highlight
            items={[
              { label: 'Division', value: rotation.newDivision },
              { label: 'Department', value: rotation.newDepartment },
              { label: 'Position', value: rotation.newPosition },
              { label: 'Supervisor', value: rotation.newSupervisor },
              { label: 'Location', value: rotation.newLocation },
            ]}
          />
        </div>

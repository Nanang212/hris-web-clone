...
import { useGetEmploymentHistories } from '@/features/employment/hooks'
import type { EmployeeEmploymentHistory } from '@/features/employment/types'
import { AppMain } from '@/shared/components/app-layout/app-main'
import { HistoryTable } from '@/features/employment/components/history-table'
...
export function EmploymentHistoryPage({
  ...
  // ─── Data ──────────────────────────────────────────────────────────────────
  const { data: histories = [], isPending, isFetching, error, refetch } = useGetEmploymentHistories(
      ...
      {activeView === 'list' && (
        <div className='flex flex-col gap-6'>
          ...
          <div className='grid grid-cols-1 gap-4 sm:grid-cols-3'>
            {[
              { label: 'Total Employees', value: totalEmployees, icon: <IconUsers size={18} />, color: 'bg-purple-600' },
              { label: 'Total Timeline Events', value: totalEvents, icon: <IconClock size={18} />, color: 'bg-blue-600' },
              { label: 'Attached Documents', value: totalDocs, icon: <IconFileText size={18} />, color: 'bg-emerald-600' },
            ].map((stat) => (
              <div
                key={stat.label}

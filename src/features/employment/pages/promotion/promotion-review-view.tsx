// promotion-review-view.tsx — Review & Confirm page before submitting promotion
...
export function PromotionReviewView({
  ...
  onSubmit,
  isPending = false,
}: PromotionReviewViewProps) {
  const employee = employees.find((e) => e.id === formData.employeeId)

  const currentDetails = [
    employee?.positionName,
    employee?.gradeName || 'G3',
  ]
    .filter(Boolean)
    .join(' · ')

  const proposedDetails = [formData.newPosition, formData.newGrade]
    .filter(Boolean)
    .join(' · ')
      ...
      <div className='rounded-2xl border border-border/60 bg-card shadow-sm overflow-hidden'>
        ...
        <div className='mx-6 mb-6 mt-2 rounded-xl border border-amber-200 bg-amber-50/60 px-4 py-3 dark:border-amber-800/40 dark:bg-amber-950/20'>
          <p className='text-xs text-amber-800 dark:text-amber-300'>
            Submitting starts the approval route. Position, grade, and salary changes become effective after approval.
          </p>
        </div>
      </div>

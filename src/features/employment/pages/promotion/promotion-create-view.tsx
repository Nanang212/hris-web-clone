// promotion-create-view.tsx — Form to create a new promotion request
...
interface PromotionCreateViewProps {
  employees: Employee[]
  onCancel: () => void
  onReview: (formData: PromotionFormData) => void
  isPending?: boolean
...
function FieldGroup({ children }: { children: React.ReactNode }) {
  return <div className='grid grid-cols-1 md:grid-cols-2 gap-4 lg:grid-cols-3'>{children}</div>
}
...
export function PromotionCreateView({
  ...
  const [newSalaryInput, setNewSalaryInput] = useState(initialData?.newSalary ? String(initialData.newSalary) : '')
  const [effectiveDate, setEffectiveDate] = useState(initialData?.effectiveDate ?? '')
  const [reason, setReason] = useState(initialData?.reason ?? '')
  const [approvalRoute, setApprovalRoute] = useState(initialData?.approvalRoute ?? 'Manager → HR → Director')

  const selectedEmployee = employees.find((e) => e.id === employeeId)
  const currentSalary = 18000000 // mock current salary

  const canSubmit =
    employeeId && newPosition && newGrade && newSalaryInput &&
    effectiveDate && reason && approvalRoute
  ...
  return (
    <div className='flex flex-col gap-6'>
      ...
      {/* Promotion Proposal Section */}
      <div className='rounded-2xl border border-border/60 bg-card shadow-sm p-6'>
        <h3 className='text-sm font-bold text-foreground mb-4 border-b pb-2'>Promotion Proposal</h3>
        ...
        <div className='flex flex-col gap-4'>
          ...
          <div className='mt-2 rounded-xl border border-emerald-100 bg-emerald-50/60 px-4 py-3 dark:border-emerald-800/40 dark:bg-emerald-950/20 flex items-center gap-3'>
      ...
      <div className='rounded-2xl border border-border/60 bg-card shadow-sm p-6'>
        <h3 className='text-sm font-bold text-foreground mb-4 border-b pb-2'>Approval Route</h3>

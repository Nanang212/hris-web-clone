...
import { useCreateEmployee, useGetEmployeeById, useUpdateEmployee } from '@/features/employment/hooks'
import type { ContractType, EmployeeStatus } from '@/features/employment/types'
...
export function EmployeeFormPage({ mode, employeeId }: Readonly<EmployeeFormPageProps>) {
  ...
  const formSchema = useSchema((z, m) => ({
    ...
    status: z.enum(['active', 'inactive', 'probation', 'resigned', 'terminated'] as const).optional(),
  }))
    ...
    if (isEdit && employeeId) {
      updateEmployee(
        { id: employeeId, payload: { ...payload, status: values.status as EmployeeStatus } },
        {
          onSuccess: () => {
            ...
            navigate({ to: '/company/employee/$id', params: { id: employeeId } })
          },
      ...
      createEmployee(payload, {
        onSuccess: (res) => {
          ...
          navigate({ to: '/company/employee/$id', params: { id: res.data.id } })
        },
        onError: (err) => snackbar.exception(err),
      })
    }
  ...
  return (
    <AppMain
      ...
      subtitle={
        isEdit
          ? m.employment_form_edit_subtitle()
          : m.employment_form_create_subtitle()
      }
      actions={

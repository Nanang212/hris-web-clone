...
import { IconFileCheck, IconCheck, IconClock } from '@tabler/icons-react'
import { useState } from 'react'
import type { EmployeeResignation, ResignationClearanceItem, ResignationStatus } from '@/features/employment/types'
...
const STATUS_CONFIG: Record<ResignationStatus, { label: string; variant: 'amber' | 'green' | 'red' | 'blue' | 'default' }> = {
...
function InfoField({ label, value }: InfoFieldProps) {
  return (
    <div className='flex flex-col gap-1.5'>
      ...
      <Input value={value} readOnly className='bg-background/50 text-foreground text-sm' />
    </div>
  )
}
  ...
  return (
    <>
      ...
      <div className='flex flex-wrap items-center justify-between gap-4 bg-muted/20 border border-border/60 rounded-2xl p-5 mb-6 text-xs'>
        <div>
          <h4 className='font-bold text-foreground text-sm'>{resignation.fullName}</h4>
        ...
        <div className='flex items-center gap-2'>
          {saveState === 'draft' && (
            <span className='inline-flex items-center gap-1 text-[11px] font-semibold text-amber-600 bg-amber-50 border border-amber-200 rounded-full px-2.5 py-0.5 dark:bg-amber-950/30 dark:border-amber-700/40 dark:text-amber-400'>
              <IconClock size={11} />
              *Draft · Belum Complete
            </span>
          )}
          {saveState === 'completed' && (
            <span className='inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-600 bg-emerald-50 border border-emerald-200 rounded-full px-2.5 py-0.5 dark:bg-emerald-950/30 dark:border-emerald-700/40 dark:text-emerald-400'>
              <IconCheck size={11} />
              ✓ Progress Complete
            </span>
          )}
          <Badge variant={status.variant as 'amber' | 'green' | 'red' | 'blue' | 'default'} className='text-xs px-3 py-1 font-semibold'>
            {status.label}
          </Badge>
        </div>
      </div>
      ...
      <div className='grid grid-cols-1 gap-6 lg:grid-cols-3'>
        <div className='lg:col-span-2 rounded-2xl border border-border/60 bg-card shadow-sm overflow-hidden'>
          ...
          <div className='p-6 grid grid-cols-1 md:grid-cols-2 gap-4'>
            ...
            <InfoField label='Exit Interview' value={resignation.exitInterviewDate ? `Scheduled · ${resignation.exitInterviewDate}` : 'Not scheduled'} />
          ...
          <div className='p-6'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
              {checklist.map((item) => (
                ...
                >
                  <div
                    className={`size-5 rounded flex items-center justify-center border-2 flex-shrink-0 transition-colors duration-150 ${
                      item.checked
                  ...
                  <span
                    className={`text-xs transition-colors duration-150 ${
                      item.checked ? 'text-emerald-700 dark:text-emerald-300 font-semibold' : 'text-muted-foreground'
                    }`}
                  >
                    {item.label}
                  </span>
        ...
        <div className='flex flex-col gap-4'>
          <div className='flex flex-col gap-3 rounded-2xl border border-border bg-card p-5 shadow-sm'>
            <h3 className='text-sm font-bold text-foreground border-b pb-3'>Resignation Actions</h3>
            <div className='flex flex-col gap-2'>
              {isActive && (
                <Button
                  variant='outline'
                  size='sm'
                  className='w-full justify-start text-xs font-semibold hover:bg-primary hover:text-primary-foreground hover:border-primary transition-colors duration-200'
                  onClick={onReview}
                >
            ...
            <div className='mt-2 rounded-xl bg-muted/40 px-4 py-3'>
              <p className='text-xs text-muted-foreground mb-2 font-semibold'>Clearance progress</p>
              <div className='flex flex-col gap-1.5'>
                {checklist.map((item) => (

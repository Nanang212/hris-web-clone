...
import type { EmployeePromotion, PromotionStatus } from '@/features/employment/promotion/types'
  ...
  return (
    <>
      ...
      <div className='mb-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-border/60 bg-muted/20 p-5 text-xs'>
        <div>
          <h4 className='text-sm font-bold text-foreground'>{promotion.fullName}</h4>
      ...
      <div className='grid grid-cols-1 gap-6 lg:grid-cols-3'>
        ...
        <div className='overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm lg:col-span-2'>
          ...
          <div className='p-6'>
            ...
            <div className='rounded-xl border border-border/40 bg-muted/20 px-4 py-1'>
              <AssignmentRow
                label='Position'
                current={promotion.currentPosition}
                proposed={promotion.newPosition}
              />
              <AssignmentRow
                label='Grade'
                current={promotion.currentGrade}
                proposed={promotion.newGrade}
              />
              <AssignmentRow
                label='Salary'
                current={formatSalary(promotion.currentSalary)}
                proposed={formatSalary(promotion.newSalary)}
              />
            </div>
        ...
        <div className='flex flex-col gap-4'>
          <div className='flex flex-col gap-3 rounded-2xl border border-border bg-card p-5 shadow-sm'>
            <h3 className='border-b pb-3 text-sm font-bold text-foreground'>Promotion Actions</h3>

...
import type { EmployeeResignation, ResignationStatus } from '@/features/employment/types'
      ...
      <Table>
        ...
        <TableBody>
          ...
          {paginated.map((res, index) => {
            const status = STATUS_CONFIG[res.status] ?? { label: res.status, variant: 'default' as const }
            ...
            return (
              <TableRow key={res.id} className='hover:bg-muted/30 transition-colors'>
                  ...
                  <div className='flex items-center gap-3'>
                    <Avatar className='size-8'>
                      ...
                      <AvatarFallback className='text-xs bg-rose-100 text-rose-700 font-bold'>
                        {res.fullName.split(' ').map((n) => n[0]).slice(0, 2).join('')}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                ...
                <TableCell>
                  <Badge variant={status.variant as 'amber' | 'green' | 'red' | 'blue' | 'default'}>{status.label}</Badge>
                </TableCell>

...
export function ResignationCreateView({
  ...
  const [resignationType, setResignationType] = useState(
    initialData?.resignationType ?? 'Voluntary',
  )
  ...
  const [accessRevocation, setAccessRevocation] = useState(
    initialData?.accessRevocation ?? 'After last working date',
  )
  ...
  return (
    <div className='flex flex-col gap-6'>
        ...
        <div className='flex flex-col gap-1.5'>
          ...
          {selectedEmployee && (
            <p className='mt-1 text-xs text-muted-foreground'>
              NIP {selectedEmployee.employeeCode} · {selectedEmployee.positionName} ·{' '}
      ...
      <div className='rounded-2xl border border-border/60 bg-card p-6 shadow-sm'>
        <h3 className='mb-4 border-b pb-2 text-sm font-bold text-foreground'>
          Resignation Information
        </h3>
        ...
        <div className='flex flex-col gap-4'>
          ...
          <div className='mt-2 rounded-xl border border-rose-200 bg-rose-50/60 px-4 py-3 dark:border-rose-800/40 dark:bg-rose-950/20'>
            <p className='text-xs text-rose-700 dark:text-rose-300'>
              Submitting schedules offboarding and access revocation. The employee stays Active
              until the last working date.
            </p>
          </div>
        </div>
      </div>

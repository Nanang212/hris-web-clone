...
let mockEmployees: Employee[] = [
  {
    id: 'emp-10021',
    employeeCode: 'EMP-10021',
    fullName: 'Dini Rahma',
    email: 'dini.rahma@company.com',
    phone: '081234567801',
    photo: null,
  ...
  {
    id: 'emp-10037',
    employeeCode: 'EMP-10037',
    fullName: 'Faaza Putri',
    email: 'faaza.putri@company.com',
    phone: '081234567802',
    photo: null,
    departmentId: 'dept-field-ops',
    departmentName: 'Field Operations',
    divisionId: null,
    divisionName: null,
    positionId: 'pos-ops-2',
    positionName: 'Field Operations Lead',
    gradeId: 'grade-3',
    gradeName: 'Grade 3',
    status: 'active',
    contractType: 'permanent',
  ...
  {
    id: 'emp-10042',
    employeeCode: 'EMP-10042',
    fullName: 'Rama Aditya',
    email: 'rama.aditya@company.com',
    phone: '081234567803',
    photo: null,
...
): Promise<Envelope<PaginatedData<Employee>>> => {
  await sleep()
...
export const getEmployeeEmploymentInfo = async (
  id: string,
...
export const getEmployeePayrollInfo = async (
  id: string,
): Promise<Envelope<EmployeePayrollInfo>> => {
  await sleep()
...
export const getEmployeeLeaveBalance = async (
  id: string,
): Promise<Envelope<EmployeeLeaveBalance[]>> => {
  await sleep()
  return {
    success: true,
    code: '200',
...
export const getContracts = async (
  search?: string,
  status?: string,
): Promise<Envelope<EmployeeContract[]>> => {
  await sleep()
  ...
  if (search) {
    ...
    filtered = filtered.filter(
      (c) => c.fullName.toLowerCase().includes(q) || c.contractNumber.toLowerCase().includes(q),
    )
  }
...
// ─── Rotation Dummy Data ─────────────────────────────────────────────────────
let mockRotations: EmployeeRotation[] = [
  {
    id: 'mut-1',
    employeeId: 'emp-1',
    employeeCode: 'EMP001',
...
export const getRotations = async (
  search?: string,
  status?: string,
): Promise<Envelope<EmployeeRotation[]>> => {
  await sleep()
  let filtered = [...mockRotations]
  if (search) {
    const q = search.toLowerCase()
    filtered = filtered.filter(
      (m) =>
...
export const createRotation = async (
  payload: CreateRotationPayload,
): Promise<Envelope<EmployeeRotation>> => {
  await sleep()
  const employee = mockEmployees.find((e) => e.id === payload.employeeId)
  const newRotation: EmployeeRotation = {
    id: `mut-${Date.now()}`,
  ...
  mockRotations = [newRotation, ...mockRotations]
  return { success: true, code: '200', data: newRotation, messages: [] }
}
...
let mockDemotions: EmployeeDemotion[] = mockPromotions.map((promotion, index) => ({
  ...promotion,
  id: `dem-${index + 1}`,
  newPosition: promotion.currentPosition.replace('Senior ', ''),
  newGrade: promotion.currentGrade,
  newSalary: Math.max(promotion.currentSalary - 2500000, 0),
  reason: index % 2 === 0 ? 'Role scope adjustment' : 'Organization structure alignment',
}))

export const getDemotions = async (
  search?: string,
  status?: string,
): Promise<Envelope<EmployeeDemotion[]>> => {
  await sleep()
  let filtered = [...mockDemotions]
  if (search) {
    const q = search.toLowerCase()
    filtered = filtered.filter(
      (p) =>
...
export const createDemotion = async (
  payload: CreateDemotionPayload,
): Promise<Envelope<EmployeeDemotion>> => {
  await sleep()
  const employee = mockEmployees.find((e) => e.id === payload.employeeId)
  const newDemotion: EmployeeDemotion = {
    id: `dem-${Date.now()}`,
    employeeId: payload.employeeId,
    employeeCode: employee?.employeeCode ?? 'EMP000',
    fullName: employee?.fullName ?? 'Unknown',
  ...
  mockDemotions = [newDemotion, ...mockDemotions]
  return { success: true, code: '200', data: newDemotion, messages: [] }
}

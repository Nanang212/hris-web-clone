// src/features/employment/employee-profile/store/employee-mcu-store.ts
...
export interface McuStore {
  ...
  getAllRecordsWithStatus: () => Array<
    McuEmployeeRecord & { latestMcu?: McuRecord; dueStatus: McuDueStatus }
  >
  ...
  persist(
    (set, get) => ({
      ...
      getLatestMcu: (employeeId) => {
        ...
        return emp.history.sort(
          (a, b) => new Date(b.mcuDate).getTime() - new Date(a.mcuDate).getTime(),
        )[0]
      },

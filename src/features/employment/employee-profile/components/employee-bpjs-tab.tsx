// src/features/employment/employee-profile/components/employee-bpjs-tab.tsx
import {
  IconCheck,
  IconEdit,
  IconExternalLink,
...
export function EmployeeBpjsTab({
  ...
  const handleSync = () => {
    ...
    setTimeout(() => {
      ...
      setIsSyncing(false)
      snackbar.success('BPJS Records successfully synchronized with Payroll BPJS configuration!')
    }, 600)
  }
        ...
        <Card className='rounded-2xl border border-border/70 shadow-xs'>
          ...
          <CardContent className='space-y-4 pt-1'>
            <div className='grid grid-cols-1 gap-3 sm:grid-cols-2'>
              ...
              <div className='space-y-1'>
                <span className='text-[11px] font-medium text-muted-foreground'>Member Since</span>
              ...
              <div className='space-y-1'>
                <span className='text-[11px] font-medium text-muted-foreground'>Contribution</span>
            ...
            <div className='mt-4 rounded-xl border border-blue-200/80 bg-blue-50/60 p-3.5 dark:border-blue-900/50 dark:bg-blue-950/30'>
              ...
              <div className='mt-2 space-y-1 text-[11px] text-muted-foreground'>
                <p>
                  ...
                  {bpjsKesConfig.companyRatePercent}% Perusahaan +{' '}
                  {bpjsKesConfig.employeeRatePercent}% Karyawan (Maks:{' '}
                  {formatIDR(bpjsKesConfig.maxWageCap)})
                </p>
                <p>
                  • <strong className='text-foreground'>BPJS TK:</strong> JHT{' '}
                  {(bpjsTkConfig.jhtCompanyPercent + bpjsTkConfig.jhtEmployeePercent).toFixed(1)}% |
                  JKK {bpjsTkConfig.jkkRatePercent}% | JKM {bpjsTkConfig.jkmRatePercent}% | JP{' '}
                  {(bpjsTkConfig.jpCompanyPercent + bpjsTkConfig.jpEmployeePercent).toFixed(1)}%
                </p>
              </div>
          ...
          <form onSubmit={handleSaveHealth} className='space-y-4 py-2'>
            ...
            <div className='space-y-1.5'>
              ...
              <Input
                ...
                onChange={(e) => setHealthForm((prev) => ({ ...prev, facility: e.target.value }))}
            ...
            <div className='grid grid-cols-2 gap-3'>
              <div className='space-y-1.5'>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDepForm((prev) => ({ ...prev, relationship: e.target.value }))
                  }
                  className='h-9 w-full rounded-xl border border-input bg-background px-3 text-xs'
                >

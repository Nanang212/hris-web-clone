...
import type { CertificateItem, AttachmentItem } from '@/features/document/types'
...
export function downloadEmployeeDocument(meta: DocumentDownloadMeta) {
  const fileName = meta.fileName || `${meta.name.replace(/\s+/g, '_')}_${meta.employeeName.replace(/\s+/g, '_')}.pdf`
 ...
 */
export function generateExcelBlob(sheetName: string, headers: string[], rows: (string | number)[][]): Blob {
...
export async function exportCertificatesBundleZip(
  ...
  const headers = ['Employee Name', 'NIP', 'Department', 'Certificate Title', 'Issuer', 'Credential ID', 'Issue Date', 'Expiry Date', 'Status']
...
export async function exportAttachmentsBundleZip(
  ...
  // 1. Add Excel Report (.xlsx)
  const headers = ['Employee Name', 'NIP', 'Department', 'File Name', 'Category', 'Uploaded Date', 'Uploaded By', 'File Size']
 ...
 */
export function downloadCsvExport(fileName: string, headers: string[], rows: (string | number)[][]) {
  const csvContent = [
    headers.join(','),

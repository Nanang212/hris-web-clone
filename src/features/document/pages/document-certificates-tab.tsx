...
import { CertificateDirectoryTable } from '@/features/document/components/certificate-directory-table'
...
export function DocumentCertificatesTab() {
  ...
  const handleExportExcel = () => {
    ...
    downloadExcelExport(
      ...
      'Certificates',
      ['Employee Name', 'NIP', 'Department', 'Certificate Title', 'Issuer', 'Credential ID', 'Issue Date', 'Expiry Date', 'Status'],
      certificates.map((c) => [
        c.fullName,
        c.employeeCode,
        c.department,
  ...
  const handleExportZipBundle = async () => {
    ...
    try {
      ...
      snackbar.success(`Downloaded complete package: Excel + ${certificates.length} Certificate PDFs!`)
    } catch {

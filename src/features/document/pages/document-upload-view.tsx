...
import { UploadCertificateModal } from '@/features/document/components/upload-certificate-modal'
import {
...
export function DocumentUploadView({
  ...
  const [selectedEmpId] = useState(
    initialEmployee?.employeeId || employees[0]?.id || 'emp-1',
  )
  ...
  const [docStates, setDocStates] = useState<LocalDocState[]>(() => {
    ...
    return [
      { key: 'ktp', name: 'KTP', required: true, fieldLabel: 'NIK', fieldValue: '3171012345678901', fileName: 'KTP_File.pdf', fileSize: '1.2 MB', status: 'verified' },
      { key: 'kk', name: 'Kartu Keluarga', required: true, fieldLabel: 'No. KK', fieldValue: '3171019988776655', fileName: 'KK_File.pdf', fileSize: '850 KB', status: 'verified' },
      { key: 'npwp', name: 'NPWP', required: true, fieldLabel: 'NPWP', fieldValue: '01.234.567.8-901.000', fileName: 'NPWP_File.pdf', fileSize: '620 KB', status: 'verified' },
      { key: 'bpjs_kes', name: 'BPJS Kesehatan', required: true, fieldLabel: 'BPJS No.', fieldValue: '', fileName: undefined, status: 'missing' },
      { key: 'bpjs_tk', name: 'BPJS Ketenagakerjaan', required: true, fieldLabel: 'BPJS No.', fieldValue: '', fileName: undefined, status: 'missing' },
      { key: 'ijazah', name: 'Ijazah Terakhir', required: true, fieldLabel: 'Education', fieldValue: 'S1 Teknik Informatika', fileName: 'Ijazah_File.pdf', fileSize: '1.8 MB', status: 'verified' },
    ]
  })
  ...
  const [isSaved, setIsSaved] = useState<boolean>(() => {
    ...
    const missing = initialEmployee.documents.filter((d) => d.required && (!d.fileName || d.status === 'missing'))
  ...
  const handleFieldValueChange = (key: string, val: string) => {
    setIsSaved(false)
    setDocStates((prev) =>
      prev.map((d) => (d.key === key ? { ...d, fieldValue: val } : d)),
    )
  }
  ...
  const handleDownloadFile = (name: string, fileName?: string, fieldLabel?: string, fieldValue?: string) => {
    downloadEmployeeDocument({
      name,
      fileName,
      fieldLabel,
  ...
  const handleSave = async () => {
    ...
    if (missingRequired.length > 0) {
      ...
      snackbar.error(`${missingRequired.length} required documents missing: upload ${missingNames} before saving.`)
      return
    }
    ...
    try {
      const res = await saveMutation.mutateAsync({
        ...
        documents: docStates.map((d) => ({
          ...
          fileName: d.fileName,
          fileSize: d.fileSize,
          status: d.fileName ? (d.required ? 'verified' : 'valid') : (d.required ? 'missing' : 'pending'),
          dueDate: d.dueDate,
        })),
      })
      setIsSaved(true)
  ...
  const handleExportDossier = async () => {
    if (!isSaved || missingRequired.length > 0) {
      snackbar.warning('Dokumen mandatory harus di-save terlebih dahulu sebelum dapat melakukan export.')
      return
    }
  ...
  return (
    <div className='flex flex-col gap-6'>
      ...
      <div className='flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-border/60 bg-card p-6 shadow-sm'>
        <div className='flex items-center gap-4'>
          ...
          <div>
            ...
            <h3 className='text-lg font-bold text-foreground mt-0.5'>{currentEmp.fullName}</h3>
        ...
        <div className='flex flex-wrap items-center gap-3'>
          <div className='flex items-center gap-3 rounded-xl border border-border/60 bg-muted/20 px-4 py-2 text-xs'>
            <div>
              ...
              <p className='text-sm font-bold text-foreground'>
                {totalDocTypes} <span className='text-[11px] font-normal text-muted-foreground'>({requiredDocs.length} mandatory, {certificates.length} certs, {attachments.length} files)</span>
              </p>
            </div>
          </div>
          ...
          <div className='flex items-center gap-3 rounded-xl border border-border/60 bg-muted/20 px-4 py-2 text-xs'>
            <div>
              ...
              <p className='text-sm font-bold text-emerald-600 dark:text-emerald-400'>
                {readyDocs.length + certificates.length + attachments.length} <span className='text-[11px] font-normal text-muted-foreground'>valid files attached</span>
              </p>
            </div>
          </div>
          ...
          <Button
            ...
            className='gap-1.5 text-xs font-semibold h-11 px-4'
            title={
      ...
      {hasAttemptedSubmit && missingRequired.length > 0 && (
        <div className='flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-rose-200 bg-rose-50/80 p-4 dark:border-rose-900/60 dark:bg-rose-950/30'>
          <div className='flex items-center gap-3'>
            <div className='flex size-8 items-center justify-center rounded-lg bg-rose-100 text-rose-600 dark:bg-rose-900/60 dark:text-rose-400 flex-shrink-0'>
          ...
          <Button
            ...
            className='border-rose-300 text-rose-700 hover:bg-rose-100 dark:border-rose-800 dark:text-rose-300 text-xs font-semibold'
          >
            Review Missing
          </Button>
        </div>
      ...
      <div className='rounded-2xl border border-border/60 bg-card p-6 shadow-sm'>
              ...
              <div
                ...
                className={`flex flex-col justify-between rounded-xl border p-4.5 transition-all ${
                  isMissingAndAttempted
                    ? 'border-rose-400 bg-rose-50/30 dark:border-rose-800 dark:bg-rose-950/10 shadow-sm'
                  ...
                  <div className='flex items-center justify-between pb-3'>
                    ...
                    <Badge
                      ...
                      className={`text-[10px] px-2 py-0.2 font-semibold ${
                        isMissingAndAttempted ? 'border-rose-500 text-rose-600 bg-rose-50' : ''
                      }`}
                    >
                    ...
                    <div className='flex items-center justify-between gap-2 rounded-lg border border-border/80 bg-muted/40 px-3 py-2 text-xs'>
                      ...
                      <div className='flex items-center gap-1 flex-shrink-0'>
                        ...
                        <button
                          ...
                          className='flex size-7 items-center justify-center rounded-md text-muted-foreground hover:bg-background hover:text-primary transition-colors'
                          title='View document'
                        >
                          <IconEye size={15} />
                        </button>
                        ...
                        <label
                          htmlFor={inputId}
                          className='flex size-7 cursor-pointer items-center justify-center rounded-md text-muted-foreground hover:bg-background hover:text-primary transition-colors'
                        ...
                        <button
                          ...
                          className='flex size-7 items-center justify-center rounded-md text-muted-foreground hover:bg-background hover:text-rose-600 transition-colors'
                          title='Remove file'
                        >
                          <IconTrash size={15} />
                        </button>
        ...
        <div className='flex flex-wrap items-center justify-between gap-3 border-t border-border/60 pt-5 mt-6'>
          <div>
            {isSaved && missingRequired.length === 0 ? (
              <p className='text-xs font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5'>
                <IconCheck size={15} className='stroke-[2.5]' />
                * Semua dokumen sudah tersave
              </p>
            ) : (
              <p className='text-xs font-semibold text-amber-600 dark:text-amber-400 flex items-center gap-1.5'>
                <IconAlertCircle size={15} />
                * Belum ada dokumen yang tersave
              </p>
            )}
            <p className='text-[11px] text-muted-foreground mt-0.5'>
              Pastikan seluruh dokumen mandatory terisi dan tersimpan sebelum melakukan ekspor berkas.
            </p>
          </div>
          ...
          <div className='flex items-center gap-3'>
            ...
            <Button size='sm' onClick={handleSave} disabled={saveMutation.isPending} className='gap-1.5 font-semibold'>
      ...
      <div className='rounded-2xl border border-border/60 bg-card p-6 shadow-sm'>
        <div className='flex flex-wrap items-center justify-between gap-3 mb-4'>
          <div>
            <div className='flex items-center gap-2'>
              ...
              <h4 className='text-sm font-bold text-foreground'>Professional Certificates ({certificates.length})</h4>
            </div>
            <p className='text-xs text-muted-foreground mt-0.5'>
              Certificates, credentials, and professional licenses registered for {currentEmp.fullName}.
            </p>
          </div>
            ...
            <Button
              ...
              className={`gap-1.5 text-xs font-semibold ${
                selectedCertIds.length === 0
                  ? 'text-rose-600 border-rose-200 hover:bg-rose-50 hover:text-rose-700 dark:border-rose-900/50 dark:hover:bg-rose-950/30'
                  : ''
              }`}
            >
        ...
        {certificates.length > 0 && (
          <div className='flex items-center justify-between bg-muted/30 rounded-lg px-3 py-1.5 mb-3 text-xs'>
        ...
        {certificates.length === 0 ? (
          <div className='rounded-xl border border-dashed border-border/70 p-6 text-center text-xs text-muted-foreground'>
            No certificates registered for this employee yet. Click &quot;Add Certificate&quot; to register one.
          </div>
        ) : (
              ...
              {paginatedCerts.map((cert: CertificateItem) => {
                ...
                return (
                  ...
                  >
                      ...
                      <div className='flex items-start justify-between gap-2 pb-2'>
                        <div className='flex items-start gap-2.5 min-w-0'>
                          <input
                            ...
                            className='size-4 mt-0.5 rounded border-border text-primary focus:ring-primary/30 cursor-pointer flex-shrink-0'
                          />
                          <div className='min-w-0'>
                            <h5 className='text-xs font-bold text-foreground truncate'>{cert.title}</h5>
                        ...
                        <Badge
                          variant={cert.status === 'active' ? 'green' : cert.status === 'expiring' ? 'amber' : cert.status === 'expired' ? 'red' : 'blue'}
                          className='text-[10px] px-2 py-0.2 capitalize flex-shrink-0'
                        >
                          {cert.status === 'expiring' ? 'Expiring Soon' : cert.status}
                        </Badge>
                      </div>

                      <p className='text-[11px] text-muted-foreground mb-3 pl-6.5'>
                    ...
                    <div className='flex items-center justify-between gap-2 rounded-lg border border-border/80 bg-muted/40 px-3 py-2 text-xs'>
                      <div className='flex items-center gap-2 min-w-0'>
                        <IconFileTypePdf size={15} className='text-purple-600 flex-shrink-0' />
                        <span className='font-semibold text-foreground truncate text-xs'>
                      ...
                      <div className='flex items-center gap-1 flex-shrink-0'>
                        ...
                        <button
                          ...
                          className='flex size-7 items-center justify-center rounded-md text-muted-foreground hover:bg-background hover:text-emerald-600 transition-colors'
      ...
      <div className='rounded-2xl border border-border/60 bg-card p-6 shadow-sm'>
        <div className='flex flex-wrap items-center justify-between gap-3 mb-4'>
          <div>
            <div className='flex items-center gap-2'>
              ...
              <h4 className='text-sm font-bold text-foreground'>Supporting Attachments ({attachments.length})</h4>
            </div>
            <p className='text-xs text-muted-foreground mt-0.5'>
        ...
        {attachments.length === 0 ? (
          <div className='rounded-xl border border-dashed border-border/70 p-6 text-center text-xs text-muted-foreground'>
            No supporting attachments uploaded for this employee yet. Click &quot;Add Attachment&quot; to upload files.
          </div>
        ) : (
          <div className='flex flex-col gap-3'>
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  ...
                  >
                    <div>
                      <div className='flex items-start justify-between gap-2 pb-2'>
                        ...
                        <Badge variant='outline' className='text-[10px] px-2 py-0.2 flex-shrink-0'>
                          {att.category}
                        </Badge>
                      </div>
                    </div>
                    ...
                    {/* Attached File Pill */}
                    <div className='flex items-center justify-between gap-2 rounded-lg border border-border/80 bg-muted/40 px-3 py-2 text-xs mt-2'>
                      <div className='flex items-center gap-2 min-w-0'>
                        <IconFileTypePdf size={15} className='text-blue-600 flex-shrink-0' />
                        <span className='font-semibold text-foreground truncate text-xs'>
                          {att.fileName}
                        </span>
                        <span className='text-[10px] text-muted-foreground flex-shrink-0'>

...
export function SchedulerLoading() {
  return (
    <div className='flex min-h-96 flex-col items-center justify-center gap-3 p-8'>
      ...
      <p className='text-sm text-muted-foreground'>Memuat jadwal kerja...</p>
    </div>
  )
}

interface SchedulerErrorProps {
  error: Error
  reset: () => void
}

export function SchedulerError({ error, reset }: Readonly<SchedulerErrorProps>) {
  return (
